package integral.studios.hydro.model.gui.impl.check.movement.impl;
...
/**
 * @author Salers
 * made on integral.studios.hydro.model.gui.impl.check.movement.impl
 */
public class FlightGUI implements IGUI {
    @Override
    public void openToPlayer(Player player) {
        SGMenu menu = Hydro.get(GuiManager.class).create(CC.PRI + "Flight Checks", 5);
        
        for (String check : Hydro.get(CheckService.class).getAlphabeticallySortedChecks()) {
            if (!check.contains("Flight")) {
                continue;
            }
