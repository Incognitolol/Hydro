package integral.studios.hydro.model.gui.impl.check.combat.impl;
...
/**
 * @author Salers
 * made on integral.studios.hydro.model.gui.impl.check.combat.impl
 */
public class ReachGUI  implements IGUI {
    @Override
    public void openToPlayer(Player player) {
        SGMenu menu = Hydro.get(GuiManager.class).create(CC.PRI + "Reach Checks", 5);
        
        for (String check : Hydro.get(CheckService.class).getAlphabeticallySortedChecks()) {
            if (!check.contains("Reach") && !check.contains("HitBox")) {
                continue;
            }
